...
var createAndSavePerson = function(done) {
  var person = new Person({ name: 'Alisa',age:17,favoriteFoods: ["pizza","tacos","mofongo"] });
  person.save(function (err,data) {
    if (err) return "Error";
      console.log("Saved!!");
      done(null , person);
...
//    let arrayOfPeople = [{ name: 'Seidel',age:22,favoriteFoods: ["hummus","tacos","mofongo"] },
  //              { name: 'Pepy',age:19,favoriteFoods: ["pizza","sushi","mofongo"] },
    //            { name: 'Alex',age:23,favoriteFoods: ["pizza","tacos","hamburguesas"] }];
...
var findPeopleByName = function(personName, done) {
  Person.find({name:personName}).exec((err,data)=>done(null, data));
};
...
var findOneByFood = function(food, done) {
  Person.findOne({favoriteFoods:food}).exec((err,data)=>done(null, data));
...
var findPersonById = function(personId, done) {
  Person.findById({_id:personId}).exec((err,data)=>done(null, data));
};
...
var findEditThenSave = function(personId, done) {
  var foodToAdd = 'hamburger';
    //Person.findById({_id:personId}).update({})
    //exec((err,data)=>done(null, data));
  
  Person.findById({_id:personId}).exec((err,data)=>{
...
var findAndUpdate = function(personName, done) {
  var ageToSet = 20;
  Person.findOneAndUpdate({name: personName},{new: true}).exec((err,data)=>{
    data.age = ageToSet;
...
var removeById = function(personId, done) {
    Person.findByIdAndRemove(personId).exec((err,data)=>done(null, data));
};
...
var removeManyPeople = function(done) {
  ...
  Person.remove({name:nameToRemove}).exec((err,data)=>done(null, data));
};
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods:{$all:[foodToSearch]}}).sort({name:1}).limit(2).select("name favoriteFoods").exec((err,data)=>{
  console.log(data);
    done(null, data);
  });
};
